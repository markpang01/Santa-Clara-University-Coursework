...
namespace coen79_lab9
{
    ...
    // Library facilities used: cstdlib
    {
        ...
        {
            // STUDENT WORK
        	inorder(f, node_ptr->left); // Processing left subtree, then carrying out f on root
		f(node_ptr->data);
    ...
    {
        // STUDENT WORK
        postorder(f, node_ptr->left); // Process on subtrees first, then on root
    ...
    {
        // STUDENT WORK
        f(node_ptr->data); // Process on root first, then subtrees
    ...
    // Library facilities used: cstdlib
    {
        ...
        if (root_ptr != NULL)
        {
            ...
            // STUDENT WORK 
		child = root_ptr->right(); // Process om subtrees then clear
		tree_clear(child);
		delete root_ptr; // Clearing heap memory
		root_ptr = NULL;
        }
    }
    ...
    // Library facilities used: cstdlib
    {
        ...
        {
            // STUDENT WORK
        	l_ptr = tree_copy(root_ptr->left()); // Pointing to left subtree with left pointer
		r_ptr = tree_copy(root_ptr->right()); // Pointing to right subtree with right pointer
    ...
    // Library facilities used: cstdlib
    {
        if (node_ptr == NULL)
            ...
            // STUDENT WORK
        	return 1 + tree_size(node_ptr->left()) + tree_size(node_ptr->right()); // Returning size of subtrees + parent
    }
}
