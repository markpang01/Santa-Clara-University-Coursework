...
#pragma mark - Toolkit Function Implementation
    ...
    // to a copy of the removed item.
    {
        ...
        {
            // STUDENT WORK
        	removed = root_ptr->data(); // At the end of the subtree, removing largest value
    ...
    // function returns false).
    {
        ...
        {   // Target was found and there is no left subtree, so we can
            ...
            // STUDENT WORK
        	oldroot_ptr = root_ptr; // Deleting old root pointer and assigning new one to new position
    ...
    // of the target were removed.
    {
        ...
        {   // Target was found and there is no left subtree, so we can
            ...
            // STUDENT WORK
        	oldroot_ptr = root_ptr; // Deleting old root pointer and moving new one to correct spot
		root_ptr = root_ptr->right();
		delete oldroot_ptr;
		return 1;
        }
    ...
    // Header file used: bintree.h
    {
        ...
        do
        {
            ...
            {   // Go left
                ...
                // STUDENT WORK
            	if (cursor->left() == NULL){
			cursor->set_left(new binary_tree_node<Item>(entry)); // Making new node with entry and setting left link
			done = true;
		}
		else{
			cursor = cursor->left();
			...
			cursor->set_right(new binary_tree_node<Item>(entry)); // Making new node with entry and setting right link
			done = true;
		}
		else{
			cursor = cursor->right();
